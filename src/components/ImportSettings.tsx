...
import { Plus, Trash2, Settings2, Table, FileSpreadsheet, Upload, X, HelpCircle, Info } from 'lucide-react';

export const ImportSettings = () => {
  const { importConfig, setImportConfig } = useInvestmentStore();
  const [previewData, setPreviewData] = useState<any[][] | null>(null);

  const handleUpdateResumo = (field: 'resumoRow' | keyof typeof importConfig.resumoCols, value: number) => {
    if (field === 'resumoRow') {
      setImportConfig({ ...importConfig, resumoRow: value });
    } else {
      setImportConfig({
        ...importConfig,
        resumoCols: { ...importConfig.resumoCols, [field]: value }
      });
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar border-r border-white/5">
        ...
        <section className="bg-primary/5 border border-primary/20 rounded-3xl p-6 relative overflow-hidden">
          ...
          <div className="space-y-3 text-sm text-white/70 relative z-10">
            ...
            <p><strong className="text-white">3. Mapeie:</strong> Use os números das <span className="text-primary font-bold">colunas</span> e <span className="text-primary font-bold">linhas</span> indicados no visualizador para preencher os campos.</p>
            <p><strong className="text-white">4. Saldo:</strong> O "Resumo" busca os totais consolidados da sua conta em uma linha específica.</p>
          </div>
        </section>

        <section className="bg-card border border-white/10 rounded-3xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputGroup 
              label="Linha do Resumo" 
              value={importConfig.resumoRow} 
              onChange={(v) => handleUpdateResumo('resumoRow', v)} 
              helper="Número da linha onde estão os totais"
            />
            <InputGroup 
              label="Coluna Total Investido" 
              value={importConfig.resumoCols.total_investido} 
              onChange={(v) => handleUpdateResumo('total_investido', v)} 
            />
            <InputGroup 
              label="Coluna Saldo Disponível" 
              value={importConfig.resumoCols.saldo_disponivel} 
              onChange={(v) => handleUpdateResumo('saldo_disponivel', v)} 
            />
            <InputGroup 
              label="Coluna Saldo Projetado" 
              value={importConfig.resumoCols.saldo_projetado} 
              onChange={(v) => handleUpdateResumo('saldo_projetado', v)} 
            />
          </div>
        </section>
            ...
            {importConfig.sections.map((section) => (
              <div key={section.id} className="bg-card border border-white/10 rounded-3xl p-6 space-y-6 relative group">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-6 border-t border-white/5">
                  ...
                  <MappingInput label="Cotação" value={section.mapping.price} onChange={(v) => handleUpdateMapping(section.id, 'price', v)} />
                  <MappingInput label="Posição" value={section.mapping.position} onChange={(v) => handleUpdateMapping(section.id, 'position', v)} />
                  <MappingInput label="Alocação %" value={section.mapping.allocation} onChange={(v) => handleUpdateMapping(section.id, 'allocation', v)} />
                </div>
...
const InputGroup = ({ label, value, onChange, onChangeText, type = 'number', helper }: { label: string, value: any, onChange?: (v: number) => void, onChangeText?: (v: string) => void, type?: string, helper?: string }) => (
  <div>
